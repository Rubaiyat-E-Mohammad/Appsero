...
/* ------------------------ Login & Dashboard ------------------------ */
test("Login & Getting Dashboard Overview Details ", async ({ page }) => {
...
/* ------------------------ Plugin ------------------------ */
test("Plugin Create & Details @fast", async ({ page }) => {
...
test("Theme", async ({ page }) => {
    ...
    theme.pro_theme_create(
        data.theme_data.website_url,
        data.theme_data.product_name
    );
...
test("Bundle", async ({ page }) => {
    ...
    let plugin_name: string = "Automated Plugin 2";
    ...
    let bundle_name: string = ""; //Any valid updateable bundle name
